...
//       {isEditing && (
//         <div className="mt-4">
//           <FileUpload onChange={(urls) => onSubmit({ urls })} />
//           {isUploading && (
  ...
  return (
    <div className="mt-6 border bg-slate-100 shadow-sm rounded-lg p-5">
      <div className="font-medium flex items-center justify-between text-gray-800">
        ...
        >
          {isEditing ? (
            ...
            <>
              <PlusCircle className="h-5 w-5 mr-2 text-blue-600" /> Add files
            </>
          )}
        </Button>
      </div>
      ...
      {!isEditing && initialData.attachments.length === 0 && (
        <p className="text-sm mt-3 text-gray-500 italic">No attachments yet</p>
      )}
      ...
      {initialData.attachments.length > 0 && (
        <div className="space-y-3 mt-3">
          {initialData.attachments.map((attachment) => (
            <div
              key={attachment.id}
              className="flex items-center p-3 w-full bg-gray-100 border border-gray-200 text-gray-700 rounded-md transition hover:bg-gray-200"
            >
              <File className="h-5 w-5 mr-3 text-blue-600 flex-shrink-0" />
              <p className="text-sm font-medium truncate">{attachment.name}</p>
              {deletingId === attachment.id ? (
                <Loader2 className="h-5 w-5 animate-spin text-red-500 ml-auto" />
              ) : (
                <button
                  onClick={() => onDelete(attachment.id)}
                  className="ml-auto p-1 rounded-full hover:bg-red-100 transition"
                >
